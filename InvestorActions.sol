...
/**
  ...
  *
  * 这是执行投资者相关操作的基金合约的支持模块
  * 如申购，赎回，分配更改和提款。 通过执行检查，
  * 执行计算并将更新后的变量返回到基金合同，本模块
  * 可能会在基金合约成立后升级。
  */

contract IInvestorActions {
  function modifyAllocation(address _addr, uint _allocation)
    public returns(uint _ethTotalAllocation) {}

  function getAvailableAllocation(address _addr)
    public returns(uint ethAvailableAllocation) {}
  ...
  function redeem(address _addr)
    public returns(uint, uint, uint, uint, uint, uint, uint) {}
...
contract InvestorActions is DestructibleModified {
  ...
  function InvestorActions() public {}
    ...
    returns(uint, uint) 
  {
    ...
    return(
      ethPendingSubscription.add(_amount),                                 // new investor.ethPendingSubscription
    ...
    returns(uint, uint, uint, uint) 
  {
    ...
    return(
      0,                                                                  // new investor.ethPendingSubscription
    ...
    returns(uint, uint) 
  {
    ...
    return(
      sharesPendingRedemption.add(_shares),                                // new investor.sharesPendingRedemption
  ...
  // 处理投资者的赎回请求并在当前的navPerShare上注销其股份
  function redeem(address _addr)
    public 
    onlyFund 
    constant 
    ...
    returns(uint, uint, uint) 
  {
    ...
    return(
      ethPendingWithdrawal,                                               // payment to be sent
